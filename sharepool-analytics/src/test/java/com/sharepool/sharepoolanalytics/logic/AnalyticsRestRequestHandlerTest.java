...
public class AnalyticsRestRequestHandlerTest {
    ...
    private void insertAnalyticsMessages() {
        analyticsMessageRepository.save(new AnalyticsMessage(1L, 2L, 4L, 41.3, 1.2, LocalDateTime.now().minusDays(3).toEpochSecond(ZoneOffset.UTC)));
        analyticsMessageRepository.save(new AnalyticsMessage(2L, 3L, 4L, 23.0, 0.75, LocalDateTime.now().minusDays(3).toEpochSecond(ZoneOffset.UTC)));

        analyticsMessageRepository.save(new AnalyticsMessage(3L, 6L, 4L, 102.5, 5.1, LocalDateTime.now().minusDays(2).toEpochSecond(ZoneOffset.UTC)));
        analyticsMessageRepository.save(new AnalyticsMessage(4L, 8L, 4L, 9.7, 0.45, LocalDateTime.now().minusDays(2).toEpochSecond(ZoneOffset.UTC)));

        analyticsMessageRepository.save(new AnalyticsMessage(5L, 10L, 4L, 65.8, 3.5, LocalDateTime.now().minusDays(1).toEpochSecond(ZoneOffset.UTC)));
    }
}