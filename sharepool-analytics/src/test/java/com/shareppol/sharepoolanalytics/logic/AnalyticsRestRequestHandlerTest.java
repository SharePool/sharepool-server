...
public class AnalyticsRestRequestHandlerTest {
    ...
    private void insertAnalyticsMessages() {
        analyticsMessageRepository.save(new AnalyticsMessage(1L, 2L, payerId, 41.3, 1.2, LocalDateTime.now().minusDays(3)));
        analyticsMessageRepository.save(new AnalyticsMessage(2L, 3L, payerId, 23.0, 0.75, LocalDateTime.now().minusDays(3)));

        analyticsMessageRepository.save(new AnalyticsMessage(3L, 6L, payerId, 102.5, 5.1, LocalDateTime.now().minusDays(2)));
        analyticsMessageRepository.save(new AnalyticsMessage(4L, 8L, payerId, 9.7, 0.45, LocalDateTime.now().minusDays(2)));

        analyticsMessageRepository.save(new AnalyticsMessage(5L, 10L, payerId, 65.8, 3.5, LocalDateTime.now().minusDays(1)));
    }
